...
class _DefaultSnapshot(tuple[ChildNode], Generic[ParentNode, ChildNode]):  # noqa: SLOT001
    ...
    def __new__(
        cls, nodes: Iterable[ChildNode], storage: ChildrenStorage[ParentNode, ChildNode] | None
    ) -> Self:
        return super().__new__(cls, nodes)  # type: ignore[arg-type]

    def __init__(
        self, nodes: Iterable[ChildNode], storage: ChildrenStorage[ParentNode, ChildNode] | None
    ) -> None:
...
class EntrySupportDefault(
    ...
    # TODO: Review
    __EMPTY = _StorageRef(None, ChildrenStorage[ParentNode, ChildNode](_fake=True), weak=True)
    ...
    def __init__(self, children: _ChildrenEntrySupportInterface[ParentNode, ChildNode]) -> None:
        ...
        self.__storage: _StorageRef[ParentNode, ChildNode] = EntrySupportDefault[
    ...
    # OK, Match
    def _notify_remove(
        self, nodes: Collection[ChildNode], current: Sequence[ChildNode]
    ...
    def _register_children_storage(
        self, storage: ChildrenStorage[ParentNode, ChildNode], *, weak: bool
    ) -> None:
        with EntrySupportDefault.__LOCK:
            if (
    ...
    def _finalised_children_storage(
        self, caller: ReferenceType[ChildrenStorage[ParentNode, ChildNode]]
    ) -> None:
